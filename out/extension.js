...
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('cypressTestHelpers.toggleOnly', toggleOnly), vscode.commands.registerCommand('cypressTestHelpers.toggleTimes', toggleTimes));
}
function findTestLine(doc, fromLine) {
    for (let i = fromLine; i >= 0; i--) {
        if (TEST_BLOCK_RE.test(doc.lineAt(i).text))
            return i;
    }
    return null;
}
...
async function wrapWithTimes(editor, itLineNum, count) {
    const doc = editor.document;
...
async function unwrapTimes(editor, timesLineNum, itLineNum) {
    const doc = editor.document;
